...
namespace BookReaderApp.Services
{
    public class DriveServiceHelper
    {
        ...
        private static readonly string[] Scopes = { DriveService.Scope.Drive }; // Thay đổi để có quyền đầy đủ
        ...
        public async Task<string> UploadFileAsync(string localFilePath, IProgress<double>? progress = null)
        {
            try
            {
                ...
                var fileId = request.ResponseBody?.Id ?? throw new Exception("Upload failed: No file ID returned.");

                // Tự động set public permission
                await SetPublicPermissionAsync(fileId);
        ...
        /// <summary>
        /// Thiết lập quyền truy cập public cho file
        /// </summary>
        ...
        private async Task SetPublicPermissionAsync(string fileId)
        {
            try
            {
                ...
                Console.WriteLine($"File {fileId} đã được set public thành công");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Warning: Không thể set public permission cho file {fileId}: {ex.Message}");
